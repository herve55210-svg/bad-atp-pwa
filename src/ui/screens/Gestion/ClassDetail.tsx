...
updated.normalizedKey = normalizeKey(`${updated.lastName}_${updated.firstName}`);
...
      <strong>📥 CSV simple</strong> — format <code>NOM;Prénom</code>, une ligne par élève.<br />
      <strong>✏️ Modifier</strong> — appuie sur le crayon pour corriger le nom ou prénom d'un élève.
    </div>
  </div>
...
            title="Modifier"
            onClick={() => setEditState({ student: s, field: 'lastName' })}
          >
            ✏️
          </button>
          {/* Bouton supprimer */}
          <button
            className="ghost"
            style={{ fontSize: 18, padding: '6px 8px', color: '#C0392B' }}
...
  {/* Modale modifier nom */}
  {editState?.field === 'lastName' && (
    <InputModal
      title={`Modifier ${editState.student.firstName} ${editState.student.lastName}`}
      label="Nom de famille"
      placeholder="DUPONT"
      defaultValue={editState.student.lastName}
      onConfirm={val => {
        setEditState({ student: { ...editState.student, lastName: val }, field: 'firstName' });
        renameStudent(editState.student, 'lastName', val);
      }}
      onClose={() => setEditState(null)}
    />
  )}
  {/* Modale modifier prénom */}
  {editState?.field === 'firstName' && (
    <InputModal
      title={`Modifier prénom de ${editState.student.lastName}`}
      label="Prénom"
      placeholder="Emma"
      defaultValue={editState.student.firstName}
      onConfirm={val => {
        renameStudent(editState.student, 'firstName', val);
        setEditState(null);
      }}
      onClose={() => setEditState(null)}
    />
