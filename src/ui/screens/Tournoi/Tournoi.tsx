...
// Supprime toutes les sessions et leurs données liées
...
<p className="page-subtitle">Sélectionne une classe puis un cycle</p>

```
  {classes.length === 0 ? (
    <div className="empty">
      <div className="empty-icon">🏫</div>
      <p>Aucune classe. Crée d'abord une classe dans l'onglet Classes.</p>
    </div>
  ) : (
    <>
...
          <div className="empty-icon">🏆</div>
          <p>Aucun cycle pour cette classe.<br />Crée un nouveau cycle.</p>
        </div>
      ) : (
        tournaments.map(t => (
...
            <button
              className="ghost"
              style={{ fontSize: 18, padding: '8px 10px', color: '#C0392B' }}
...
      message={`Supprimer "${deleteTarget.name}" et toutes ses séances ? Cette action est irréversible.`}
